...
- **`nx build web` (production static export) is currently BLOCKED — both bundlers fail.** Dev mode (`nx serve web`, `next dev --webpack`) works fine; this only affects the production build. Two independent failures:
  1. **`--webpack` (current):** Next 16.1.7 static-generation throws `TypeError: Cannot read properties of null (reading 'useContext')` (React is null in the SSG worker) on client-heavy pages (dashboard, analyze) and the synthesized `/_global-error`. Reproduced on clean `npm ci`, cleared caches, and even a minimal context-free `global-error.tsx`, at both next@16.1.6 and 16.1.7 — it is a Next `--webpack` SSG bug, not app code. (Earlier phases' "green web build" were stale nx cache hits — web was not rebuilt cold after Phase 12.) Mitigations already applied: `next.config.js` resolves the next-intl config relative to cwd (fixes graph inference); `/analyze` is `force-dynamic`; a minimal `global-error.tsx` exists.
  2. **Turbopack (`next build`, the forward fix):** fails with 27 errors — `libs/contracts`/`libs/shared`/`libs/ui`/`libs/config` declare `"type": "commonjs"` (correct for the NestJS backend, webpack-bundled) but are authored/emitted as ESM.
  **Real fix:** migrate the frontend-consumed libs (contracts, shared, ui, config) to a Turbopack-compatible module format (drop `"type":"commonjs"` / add dual `exports` / emit matching JS) WITHOUT breaking the webpack-bundled backend (api/worker), then switch `apps/web/project.json` build back to Turbopack. Needs a dedicated task with a backend build check. *Escalated in the analyze-UI phase.*
